...
void OpticalFlow::FarnebackPolyExp(const Mat &src, Mat &dst, int n,
                                   double sigma) {
  int k, x, y;
  ...
  double ig11 = invG(1, 1), ig03 = invG(0, 3), ig33 = invG(3, 3),
         ig55 = invG(5, 5);
...
void OpticalFlow::FarnebackUpdateMatrices(const Mat &_R0, const Mat &_R1,
...
void OpticalFlow::FarnebackUpdateFlow_GaussianBlur(const Mat &_R0,
                                                   const Mat &_R1, Mat &_flow,
  ...
  for (y = 0; y < height; y++) {
    ...
    for (i = 0; i <= m; i++) {
      ...
      srow[m + i] =
          (const float *)(matM.data + matM.step * std::min(y + i, height - 1));
    }
...
void OpticalFlow::MedianBlurFlow(Mat &flow, const int ksize) {
  Mat channels[2];
  split(flow, channels);
...
void OpticalFlow::FarnebackPolyExpPyr(const Mat &img,
...
void OpticalFlow::calcOpticalFlowFarneback(std::vector<Mat> &prev_poly_exp_pyr,
                                           std::vector<Mat> &poly_exp_pyr,
  ...
  for (k = flow_pyr.size() - 1; k >= 0; k--) {
    ...
    if (!prevFlow.data)
      ...
      resize(prevFlow, flow, cv::Size(width, height), 0, 0, INTER_LINEAR);
      flow *= sqrt(2);
    }
    ...
    for (i = 0; i < iterations; i++)
      FarnebackUpdateFlow_GaussianBlur(R[0], R[1], flow, M, winsize,
                                       i < iterations - 1);
