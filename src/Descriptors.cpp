...
void Descriptors::GetRect(const Point2f &point, ParamManager::RectInfo &rect,
...
void Descriptors::BuildDescMat(const Mat &xComp, const Mat &yComp, float *desc,
                               const ParamManager::DescInfo &descInfo) {
    ...
    for (int i = 0; i < xComp.rows; i++, index += nDims) {
        ...
        for (int j = 0; j < xComp.cols; j++) {
            float x = xc[j];
            float y = yc[j];
            float mag0 = sqrt(x * x + y * y); //å˘îzã≠ìxÇÃéZèo
            float mag1;
            int bin0, bin1;
            
            // for the zero bin of hof
            if (descInfo.isHof && mag0 <= param->min_flow) {
...
void Descriptors::GetDesc(const ParamManager::DescMat *descMat,
    ...
    for (int xPos = rect.x, x = 0; x < descInfo.nxCells; xPos += xStride, x++)
        for (int yPos = rect.y, y = 0; y < descInfo.nyCells; yPos += yStride, y++) {
            ...
            for (int i = 0; i < nBins; i++) {
                float sum =
                bottom_right[i] + top_left[i] - bottom_left[i] - top_right[i];
                vec[iDesc++] = std::max<float>(sum, 0) + param->epsilon;
            }
        }
...
void Descriptors::HogComp(const Mat &img, float *desc,
                          ParamManager::DescInfo &descInfo) {
    Mat imgX, imgY;
...
void Descriptors::HofComp(const Mat &flow, float *desc,
                          ParamManager::DescInfo &descInfo) {
    Mat flows[2];
    split(flow, flows);
...
void Descriptors::MbhComp(const Mat &flow, float *descX, float *descY,
                          ParamManager::DescInfo &descInfo) {
    Mat flows[2];
    split(flow, flows);
...
bool Descriptors::IsValid(std::vector<Point2f> &track, float &mean_x,
                          float &mean_y, float &var_x, float &var_y,
                          float &length) {
    int size = track.size();
    float norm = 1. / size;
    ...
    if (var_x < param->min_var && var_y < param->min_var)
        return false;
    // remove random trajectory
    if (var_x > param->max_var || var_y > param->max_var)
        return false;
    ...
    if (cur_max > param->max_dis && cur_max > length * 0.7)
        return false;
...
bool Descriptors::IsCameraMotion(std::vector<Point2f> &disp) {
    float disp_max = 0;
    float disp_sum = 0;
...
void Descriptors::DenseSample(const Mat &grey, std::vector<Point2f> &points,
...
void Descriptors::InitPry(const Mat &frame, std::vector<float> &scales,
    ...
    int nlayers = 0;
    while (1) {
        if(min_size < param->patch_size)
            break;
        min_size /= param->scale_stride;
        nlayers++;
    }
    ...
    param->scale_num = std::min<int>(param->scale_num, nlayers);
    
    scales.resize(param->scale_num);
    sizes.resize(param->scale_num);
    
    scales[0] = 1.;
    sizes[0] = cv::Size(cols, rows);
    
    for (int i = 1; i < param->scale_num; i++) {
        scales[i] = scales[i - 1] * param->scale_stride;
        sizes[i] = cv::Size(int(cols / scales[i]), int(rows / scales[i]));
    }
}

void Descriptors::BuildPry(const std::vector<cv::Size> &sizes, const int type,
...
void Descriptors::DrawTrack(const std::vector<Point2f> &point, const int index,
    ...
    for (int j = 1; j <= index; j++) {
        ...
        line(image, point0, point1,
             Scalar(0, cvFloor(255.0 * (j + 1.0) / float(index + 1.0)), 0), 2, 8,
             0);
        point0 = point1;
    }
...
void Descriptors::PrintDesc(std::vector<float> &desc,
                            ParamManager::DescInfo &descInfo,
...
void Descriptors::StoreDesc(std::vector<float>& desc, ParamManager::DescInfo& descInfo,
                            ParamManager::TrackInfo& trackInfo, std::vector<std::vector<float>> &storeVec){
    int tStride = cvFloor(trackInfo.length/descInfo.ntCells);
    float norm = 1./float(tStride);
    int dim = descInfo.dim;
    int pos = 0;
    
    int sample_size = storeVec.size();
    
    cv::Mat temp_desc(descInfo.ntCells, dim, CV_32F);
    ...
    for(int i = 0; i < descInfo.ntCells; i++){
        ...
        for(int j = 0; j < dim; j++)
            temp_desc.at<float>(i, j) = vec[j] * norm;
        
        if(i == descInfo.ntCells - 1){
            storeVec.resize(sample_size + 1);
            
            for(int x = 0 ; x < descInfo.ntCells ; x++)
                for(int y = 0 ; y < dim ; y++)
                    storeVec[sample_size].push_back(temp_desc.at<float>(x, y));
        }
    }
}


void Descriptors::LoadBoundBox(char *file, std::vector<Frame> &bb_list) {
    ...
    while (std::getline(bbFile, line)) {
        ...
        for (int i = 0; i < size / 5; i++)
            cur_frame.BBs.push_back(BoundBox(a[i * 5], a[i * 5 + 1], a[i * 5 + 2],
                                             a[i * 5 + 3], a[i * 5 + 4]));
...
void Descriptors::InitMaskWithBox(Mat &mask, std::vector<BoundBox> &bbs) {
    int width = mask.cols;
...
void Descriptors::MyWarpPerspective(Mat &prev_src, Mat &src, Mat &dst, Mat &M0,
                                    int flags, int borderType,
    ...
    for (y = 0; y < height; y += bh0) {
        for (x = 0; x < width; x += bw0) {
            ...
            Mat matA;
            Mat dpart(dst, cv::Rect(x, y, bw, bh));
            ...
            for (y1 = 0; y1 < bh; y1++) {
                ...
                for (x1 = 0; x1 < bw; x1++) {
                    ...
                    if (_X > margin && _X < width - 1 - margin && _Y > margin &&
                        _Y < height - 1 - margin)
...
void Descriptors::ComputeMatch(const std::vector<KeyPoint> &prev_kpts,
                               const std::vector<KeyPoint> &kpts,
                               const Mat &prev_desc, const Mat &desc,
...
void Descriptors::MergeMatch(const std::vector<Point2f> &prev_pts1,
...
void Descriptors::MatchFromFlow(const Mat &prev_grey, const Mat &flow,
                                std::vector<Point2f> &prev_pts,
